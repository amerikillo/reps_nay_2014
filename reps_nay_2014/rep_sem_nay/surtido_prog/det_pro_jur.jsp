...
<div style="width:920px; margin:auto; background-color:#FFFfff">
  <div style="width:180; float:left; background-color:#FFF; padding:15px; font-family:Arial, Helvetica, sans-serif; font-size:10px;">
	...
	int t_por=0;	
	String totales ="select j.des_jur, sum(bsol.can_sol)  FROM back_solicitado bsol, unidades u, municipios m, jurisdicciones j  where u.tip_uni = 'P' and j.cla_jur = '"+juris+"' and  bsol.FEC_Sol between '"+fecha1+"' and '"+fecha2+"' and  j.cla_jur = m.cla_jur and  m.cla_mun = u.cla_mun and  u.cla_uni = bsol.cla_uni  GROUP BY j.cla_jur ;";
	rset = stmt.executeQuery(totales);
	while(rset.next()){	
		total_req = rset.getString(2);
	}
	
    totales ="select j.des_jur, sum(bsur.can_sur) FROM back_surtido bsur, unidades u, municipios m, jurisdicciones j where u.tip_uni = 'P' and j.cla_jur = m.cla_jur and j.cla_jur='"+juris+"' and m.cla_mun = u.cla_mun and bsur.FEC_SUR between '"+fecha1+"' and '"+fecha2+"' and u.cla_uni = bsur.cla_uni GROUP BY j.cla_jur ;";
	rset = stmt.executeQuery(totales);
	while(rset.next()){	
  ...
  <br />
        ...
        <%
		String qry_msol= "select m.des_mun, sum(bsol.can_sol)  FROM back_solicitado bsol, unidades u, municipios m, jurisdicciones j  where u.tip_uni = 'P' and j.cla_jur = '"+juris+"' and  bsol.FEC_Sol between '"+fecha1+"' and '"+fecha2+"' and  j.cla_jur = m.cla_jur and  m.cla_mun = u.cla_mun and  u.cla_uni = bsol.cla_uni  GROUP BY m.des_mun;";
		...
		while(rset.next()){
			...
			String m_sur = "0";
			int m_por = 0; 
			String qry_msur= "select m.des_mun, sum(bsur.can_sur)  FROM back_surtido bsur, unidades u, municipios m, jurisdicciones j  where u.tip_uni = 'P' and m.des_mun = '"+muni+"' and  bsur.FEC_Sur between '"+fecha1+"' and '"+fecha2+"' and  j.cla_jur = m.cla_jur and  m.cla_mun = u.cla_mun and  u.cla_uni = bsur.cla_uni  GROUP BY m.des_mun;";
			...
			%>
				...
				<%
				String qry_uni = "select u.des_uni, sum(bsol.can_sol)  FROM back_solicitado bsol, unidades u, municipios m, jurisdicciones j  where u.tip_uni = 'P' and m.des_mun='"+muni+"' and  bsol.FEC_Sol between '"+fecha1+"' and '"+fecha2+"' and  j.cla_jur = m.cla_jur and  m.cla_mun = u.cla_mun and  u.cla_uni = bsol.cla_uni  GROUP BY u.des_uni;";
				...
				while(rset2.next()){
					...
					String u_sur = "0";
					int u_por=0;
					String qry_usur= "select u.des_uni, sum(bsur.can_sur)  FROM back_surtido bsur, unidades u, municipios m, jurisdicciones j  where u.tip_uni = 'P' and u.des_uni = '"+uni+"' and  bsur.FEC_Sur between '"+fecha1+"' and '"+fecha2+"' and  j.cla_jur = m.cla_jur and  m.cla_mun = u.cla_mun and  u.cla_uni = bsur.cla_uni  GROUP BY u.des_uni;";
