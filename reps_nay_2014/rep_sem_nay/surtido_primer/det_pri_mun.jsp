...
<div style="width:920px; margin:auto; background-color:#FFFfff">
  <div style="width:180; float:left; background-color:#FFF; padding:15px; font-family:Arial, Helvetica, sans-serif; font-size:10px;">
	...
	int m_por;
	String totales ="select j.des_jur, sum(bsol.can_sol)  FROM back_solicitado bsol, unidades u, municipios m, jurisdicciones j  where u.tip_uni = '1' and m.des_mun = '"+muni+"' and  bsol.FEC_Sol between '"+fecha1+"' and '"+fecha2+"' and  j.cla_jur = m.cla_jur and  m.cla_mun = u.cla_mun and  u.cla_uni = bsol.cla_uni  GROUP BY m.des_mun ;";
	rset = stmt.executeQuery(totales);
	while(rset.next()){	
		total_req = rset.getString(2);
	}
	
    totales ="select j.des_jur, sum(bsur.can_sur) FROM back_surtido bsur, unidades u, municipios m, jurisdicciones j where u.tip_uni = '1' and j.cla_jur = m.cla_jur and m.des_mun = '"+muni+"' and m.cla_mun = u.cla_mun and bsur.FEC_SUR between '"+fecha1+"' and '"+fecha2+"' and u.cla_uni = bsur.cla_uni GROUP BY m.des_mun ;";
	rset = stmt.executeQuery(totales);
	while(rset.next()){	
	...
	%>
    <tr class="numeros">
      <td style="text-align:left"><strong class="tit_tablas"><%=muni%></strong></td>
  ...
  <table border="1" width="693"  bgcolor="#F2F2F2" bordercolor="#FFFFFF" cellspacing="0">
      ...
      <%
	  String qry_meds="select p.cla_pro as med, p.des_pro as descr, SUM(bsol.can_sol) as sol from jurisdicciones j, municipios m, unidades u, productos p, back_solicitado bsol  where  j.cla_jur = m.cla_jur and  m.cla_mun = u.cla_mun and  u.cla_uni = bsol.cla_uni and  bsol.cla_pro = p.cla_pro and  bsol.FEC_sol between '"+fecha1+"' and '"+fecha2+"' and  m.des_mun = '"+muni+"' and u.tip_uni = '1' group by p.cla_pro  order by p.cla_pro+0  ;";
