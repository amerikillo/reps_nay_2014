...
<div class="container">
  ...
  <%
	...
	%>
    <div> <a href="venta_total.jsp">Reporte de Ventas</a> &gt; <a href="det_venta.jsp?tipo=<%=request.getParameter("tipo")%>">Tipo <%=tip%></a> &gt; <a href="detalle_juris.jsp?tipo=<%=request.getParameter("tipo")%>&juris=<%=request.getParameter("juris")%>">Jurisdiccion <%=juris%></a> &gt; <%=request.getParameter("mun")%></div>
  </div>
  <div>
    <h2>Ventas Totales&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="gnr_detalle_mun.jsp?mun=<%=request.getParameter("mun")%>&juris=<%=request.getParameter("juris")%>&tipo=<%=request.getParameter("tipo")%>"><img src="../../imagen/Excel-2013.png" alt="Exportar a Excel" width="40" height="40" /></a></h2>
    <p>&nbsp;</p>
  </div>
    ...
    al: <input name="fecha2" type="text" id = "fecha2" value="<%=f2_mos%>" size="10" readonly /><img src="../../imagen/cal.gif" width="15" height="15" border="0" onClick="scwShow(scwID('fecha2'),event);"/>
     ...
     <a href="recetas_surtidas/gnr_rec_sur.jsp?fecha1=<%=f1_mos%>&fecha2=<%=f2_mos%>&Submit=Filtrar" target="_blank"><img src="../../imagen/exc.jpg" width="69" height="50" alt="Exportar a Excel" /></a>
      ...
      <div>
        <table width="700"  class="table table-bordered table-striped table-hover" style="font-size:10px;">
          ...
          <%
			...
			float jniv=0;
			rset = stmt.executeQuery("SELECT m.des_mun, sum(b.can_sol) from jurisdicciones j, municipios m, unidades u, folio_back f, back_solicitado b WHERE j.cla_jur = m.cla_jur and m.cla_mun = u.cla_mun and u.cla_uni = f.cla_uni AND f.fol_bac = b.fol_bac AND m.des_mun='"+(request.getParameter("mun"))+"' and u.tip_uni='"+request.getParameter("tipo")+"' GROUP BY j.cla_jur ;");
			try{
			while(rset.next()){
				jjuris=rset.getString(1);
				jsol=Integer.parseInt(rset.getString(2));
				rset2 = stmt2.executeQuery ("SELECT sum(b.can_sur) from jurisdicciones j, municipios m, unidades u, folio_back f, back_surtido b WHERE j.cla_jur = m.cla_jur and m.cla_mun = u.cla_mun and u.cla_uni = f.cla_uni AND f.fol_bac = b.fol_bac AND m.des_mun='"+(request.getParameter("mun"))+"' and u.tip_uni='"+request.getParameter("tipo")+"' GROUP BY j.cla_jur ;");
				while(rset2.next()){
        ...
        <table border="1" class="table table-bordered table-striped table-hover" style="font-size:10px;">
          ...
          <%
	  String qry_meds="SELECT p.cla_pro, p.des_pro, b.can_sol from jurisdicciones j, municipios m, unidades u, folio_back f, back_solicitado b, productos p WHERE j.cla_jur = m.cla_jur and m.cla_mun = u.cla_mun and u.cla_uni = f.cla_uni AND f.fol_bac = b.fol_bac AND b.cla_pro = p.cla_pro AND m.des_mun = '"+request.getParameter("mun")+"' and u.tip_uni = '"+request.getParameter("tipo")+"' GROUP BY p.cla_pro order by p.cla_pro+0 ;";
	  ...
	  while(rset.next()){
		  ...
		  String qry_meds2="SELECT p.cla_pro, p.des_pro, b.can_sur from jurisdicciones j, municipios m, unidades u, folio_back f, back_surtido b, productos p WHERE j.cla_jur = m.cla_jur and m.cla_mun = u.cla_mun and u.cla_uni = f.cla_uni AND f.fol_bac = b.fol_bac AND b.cla_pro = p.cla_pro AND m.des_mun = '"+request.getParameter("mun")+"' and u.tip_uni = '"+request.getParameter("tipo")+"' and p.cla_pro='"+clave+"' GROUP BY p.cla_pro order by p.cla_pro+0 ;";
