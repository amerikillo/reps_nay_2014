...
response.setHeader("Content-Disposition","attachment; filename="+unidad_jv+" por Municipio periodo del "+fecha_1+" al "+fecha_2+".xls");
    ...
    if(conexion.isClosed()){
        mensaje="desconectado";
			...
			if(!(fecha_1.equals("")) && !(fecha_2.equals(""))){
				  	rset_20 = stmt_20.executeQuery("select sum(total_sur) from juris_mpio_fecha where fec_sur between '"+fecha_1+"' and '"+fecha_2+"' and cla_jur='"+juris_jv+"' and cla_uni in ('1002', '1004','1008','1014','2002','2004','2006','2008','3002','3004','3006','3008','3010');");
				  }else{
					  rset_20 = stmt_20.executeQuery("select sum(total_sur) from juris_mpio_fecha where cla_jur='"+juris_jv+"' and cla_uni in ('1002', '1004','1008','1014','2002','2004','2006','2008','3002','3004','3006','3008','3010');");
				  }
...
<table border=0 cellpadding=0 cellspacing=0 width=439 style='border-collapse:
 ...
 <tr height=20 style='height:15.0pt'>
  ...
  <td class=xl72 align=right><%=nf1.format(totPzs)%></td>
 ...
 <%
  ...
  if(!(fecha_1.equals("")) && !(fecha_2.equals(""))){
    	rset_20 = stmt_20.executeQuery("select des_mun,sum(total_sur) from juris_mpio_fecha TOT inner join municipios M on M.cla_mun=TOT.cla_mun where fec_sur between '"+fecha_1+"' and '"+fecha_2+"' and TOT.cla_jur='"+juris_jv+"' and tot.cla_uni in ('1002', '1004','1008','1014','2002','2004','2006','2008','3002','3004','3006','3008','3010') group by TOT.cla_mun;");
	  }else{
		  rset_20 = stmt_20.executeQuery("select des_mun,sum(total_sur) from juris_mpio_fecha TOT inner join municipios M on M.cla_mun=TOT.cla_mun  where TOT.cla_jur='"+juris_jv+"' and tot.cla_uni in ('1002', '1004','1008','1014','2002','2004','2006','2008','3002','3004','3006','3008','3010') group by TOT.cla_mun;");
	  }
