...
<form name="form" action="diagtt6a12isem.jsp" method="post">
...
<table width="1015" border="1" cellpadding="2" bordercolor="#7D2322" class="ver">
                     <tr>
                       <td width="585"><table width="1005" border="1" cellpadding="0">
                         ...
                         <tr height="17">
                           <td height="26" colspan="6" background="../../../imagen/fon3.jpg" bgcolor="#F0F0F0"><div align="center"><a href="#ancla1" class="style12">DIAGNÓSTICOS PRINCIPALES EDAD DE 6 A 12 A&Ntilde;OS </a></div></td>
                         </tr>
                         <!--tr height="17">
		...
		String fechaa2_jv="";
                  ...
                  <span class="style12"><%=!(fecha_1.equals(""))?"Rango desde "+fechaa1_jv+" hasta "+fechaa2_jv:""%></span></td>
                         ...
                         <%
				 rset_200 = stmt_200.executeQuery("select cldia1,sum(cant_costo),sum(diag1) from t_diag_princ where edad in (6,7,8,9,10,11,12) group by cedis;");
if(!(fecha_1.equals("")) && !(fecha_2.equals(""))){
				  	rset_200 = stmt_200.executeQuery("select cldia1,sum(cant_costo),sum(diag1) from t_diag_princ where fecha between '"+fecha_1+"' and '"+fecha_2+"' and edad in (6,7,8,9,10,11,12) group by cedis;");
				...
				while (rset_200.next()) 
				  ...
				  rset_201 = stmt_200.executeQuery("select sum(cant_costo),sum(diag1) from t_diag_princ where edad in (6,7,8,9,10,11,12);");
if(!(fecha_1.equals("")) && !(fecha_2.equals(""))){
				  	rset_201 = stmt_200.executeQuery("select sum(cant_costo),sum(diag1) from t_diag_princ where fecha between '"+fecha_1+"' and '"+fecha_2+"' and edad in (6,7,8,9,10,11,12);");
				...
				%>
                         <%
						 rset_20 =  rset_20 = stmt_200.executeQuery("select cldia1,dia1,sum(diag1) as receta ,sum(cant_costo) from t_diag_princ where edad in (6,7,8,9,10,11,12) group by cldia1 order by  receta desc ;");
						 if(!(fecha_1.equals("")) && !(fecha_2.equals(""))){
				 rset_20 = stmt_20.executeQuery("select cldia1,dia1,sum(diag1) as receta ,sum(cant_costo) from t_diag_princ where fecha between '"+fecha_1+"' and '"+fecha_2+"' and edad in (6,7,8,9,10,11,12) group by cldia1 order by  receta desc limit 0,20  ;");
	}
...
rset_med = stmt_2000.executeQuery("select cldia1,sum(cant_costo),sum(diag1) from t_diag_princ where cldia1='"+cldia_jv+"' and edad in (6,7,8,9,10,11,12) group by cedis ;");
				
	if(!(fecha_1.equals("")) && !(fecha_2.equals(""))){
				rset_med = stmt_2000.executeQuery("select cldia1,sum(cant_costo),sum(diag1) from t_diag_princ where cldia1='"+cldia_jv+"' and fecha between '"+fecha_1+"' and '"+fecha_2+"' and edad in (6,7,8,9,10,11,12) group by cedis;");
	}
