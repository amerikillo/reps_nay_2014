...
rset_200 = stmt_200.executeQuery("SELECT MIN(FECHA),MAX(FECHA),YEAR(min(FECHA)) AS anio1,MONTH(Min(FECHA)) AS mes1 ,DAY(Min(FECHA)) AS dia1,YEAR(MAX(FECHA)) AS anio2,MONTH(MAX(FECHA)) AS mes2 ,DAY(MAX(FECHA)) AS dia2 FROM t_diag_feme;");
...
// Rutina para fechas
		 ...
		 if( (month>=1 && (years>=114 || years<=108))  || (month2>=3 && (years2>=114 || years<=108)) )
		   ...
		   ResultSet rset_d = stmt_d.executeQuery("select MAX(fecha) from t_diag_feme;"); 
                    String fechmax="";
					int banexi=1;
					while(rset_d.next()){
                    fechmax=rset_d.getString("MAX(fecha)");
					}
					rset_d = stmt_d.executeQuery("select MIN(fecha) from t_diag_feme;"); 
                    String fechmin="";
					while(rset_d.next()){
...
/*if(!(fecha_1.equals("")) && !(fecha_2.equals(""))){
				  	rset_20 = stmt_20.executeQuery("select cldia1,dia1,count(cldia1) as receta from t_diag_feme where fecha between '"+fecha_1+"' and '"+fecha_2+"'  group by cldia1 order by  receta desc limit 0,20;");
...
<form name="form" action="diagttsexisem.jsp" method="post">
...
<table width="1015" border="1" cellpadding="2" bordercolor="#7D2322" class="ver">
                     <tr>
                       <td width="585"><table width="1005" border="1" cellpadding="0">
                         ...
                         <tr height="17">
                           <td height="26" colspan="7" background="../../../imagen/fon3.jpg" bgcolor="#F0F0F0"><div align="center"><a href="#ancla1" class="style12">DIAGNÓSTICOS PRINCIPALES SEXO FEMENINO </a></div></td>
                         </tr>
                         <tr height="17">
		...
		String fechaa2_jv="";
                  ...
                  <span class="style12"><%=!(fecha_1.equals(""))?"Rango desde "+fechaa1_jv+" hasta "+fechaa2_jv:""%></span></td>
                         ...
                         <tr height="17">
                            <td width="35" height="23" bgcolor="#CCCCCC" class="dis2"><div align="center">N&deg;</div></td>
                           <td width="67" height="23" bgcolor="#CCCCCC" class="dis2"><div align="center">CIE-10</div></td>
                           <td width="586" height="23" bgcolor="#CCCCCC" class="dis2"><div align="center">DESCRIPCION</div></td>
                         ...
                         <%
				 rset_200 = stmt_200.executeQuery("select cldia1,sum(cant_costo),sum(diag1) from t_diag_feme  group by cedis;");
if(!(fecha_1.equals("")) && !(fecha_2.equals(""))){
				  	rset_200 = stmt_200.executeQuery("select cldia1,sum(cant_costo),sum(diag1) from t_diag_feme where fecha between '"+fecha_1+"' and '"+fecha_2+"'  group by cedis;");
	 			...
	 			*/
				  ...
				  /*
				  rset_201 = stmt_200.executeQuery("select sum(cant_costo) from t_diag_feme ;");
if(!(fecha_1.equals("")) && !(fecha_2.equals(""))){
				  	rset_201 = stmt_200.executeQuery("select sum(cant_costo) from t_diag_feme where fecha between '"+fecha_1+"' and '"+fecha_2+"' ;");
				...
				%>
                         <%
						 rset_20 =  rset_20 = stmt_200.executeQuery("select cldia1,dia1,sum(diag1) as receta ,sum(cant_costo) from t_diag_feme  where cldia1 not like '%999%' group by cldia1 order by  receta desc limit 0,20 ;");
						 if(!(fecha_1.equals("")) && !(fecha_2.equals(""))){
				 rset_20 = stmt_20.executeQuery("select cldia1,dia1,sum(diag1) as receta ,sum(cant_costo) from t_diag_feme  where fecha between '"+fecha_1+"' and '"+fecha_2+"' and cldia1 not like '%999%'  group by cldia1 order by  receta desc limit 0,20  ;");
	}
...
rset_med = stmt_2000.executeQuery("select cldia1,sum(cant_costo),sum(diag1) from t_diag_feme where cldia1='"+cldia_jv+"'  group by cedis ;");
				
	if(!(fecha_1.equals("")) && !(fecha_2.equals(""))){
				rset_med = stmt_2000.executeQuery("select cldia1,sum(cant_costo),sum(diag1) from t_diag_feme where cldia1='"+cldia_jv+"'  and fecha between '"+fecha_1+"' and '"+fecha_2+"' group by cedis;");
	}
				...
				//cierra del while
                         ...
                         <tr height="17">
                           <td align="center" width="35"><%=contdiag%></td>
                           <td height="17" align="right" width="67"><div align="center"><a href="medttisem_feme.jsp?med=<%=("-")%><%//=rset_20.getString("cldia1")%>"><%=("-")%><%//=rset_20.getString("cldia1")%></a></div>
                           <div align="center"></div></td>
                           <td width="586"><div align="left"><%=("-")%><%//=rset_20.getString("dia1")%></div></td>
