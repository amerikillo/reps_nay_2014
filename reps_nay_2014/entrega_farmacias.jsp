...
r_fecha = s_fecha.executeQuery("SELECT YEAR(min(fecha)) AS anio1,MONTH(Min(fecha)) AS mes1 ,DAY(Min(fecha)) AS dia1,YEAR(MAX(fecha)) AS anio2,MONTH(MAX(fecha)) AS mes2 ,DAY(MAX(fecha)) AS dia2 FROM total_juris_all_actualizacion;");
	...
	if (!(f_min.equals("")) && !(f_max.equals("")))
		...
		r_consulta = s_consulta.executeQuery("select juris,mpio,uni_ate,cv_uni,fecha from total_juris_all_actualizacion where fecha between '"+f_min+"' and '"+f_max+"' group by uni_ate,fecha;");
		...
		r_consulta = s_consulta.executeQuery("select juris,mpio,uni_ate,cv_uni,fecha from total_juris_all_actualizacion where cv_uni='0' group by uni_ate,fecha;");
		}
...
<table width="713" border="1">
  <tr>
    <td colspan="5" align="center">DISPENSADO DE Unidades </td>
    </tr>
  <tr>
  ...
  while (r_consulta.next())
  {
	  ...
	  s_insert.execute("insert into registro_por_fecha values(0,'"+clave+"',curdate(),'"+fecha+"');");
	  
	  r_consulta2 = s_consulta2.executeQuery("select cla_reg from registro_por_fecha where cla_uni='"+clave+"' and fec_sur='"+fecha+"'");
	  while(r_consulta2.next())
	   c_reg=r_consulta2.getString("cla_reg");

      r_consulta3 = s_consulta3.executeQuery("select clave,uni,costo from total_juris_all_actualizacion where cv_uni='"+clave+"' and fecha='"+fecha+"'");
	 ...
	 s_insert.execute("insert into entregado_por_unidad values('"+c_reg+"','"+medica+"','"+c_sur+"','"+c_total+"',0);");
	  }
