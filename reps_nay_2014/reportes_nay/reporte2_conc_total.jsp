...
//objeto de la Clase Connection, nos sirve para hacer la conexión a la base correspondiente
   ...
   /*parametros para la conexion*/
	...
	response.setHeader("Content-Disposition","attachment; filename=REPORTE N° 2.xls");
...
{
        rset_001=stmt_001.executeQuery("select clave,descri,present,sum(uni),costo_u,sum(costo) from concentrado_2_3_4_total where fecha between '"+ran1+"' and '"+ran2+"' and partida='1' group by clave order by (clave+0)");
		
       rset_002=stmt_002.executeQuery("select sum(uni),sum(costo),costo_u from concentrado_2_3_4_total where fecha between '"+ran1+"' and '"+ran2+"' and id_med='MEDICAMENTO'  and  partida='1' group by clave");
	}else{
 rset_001=stmt_001.executeQuery("select clave,descri,present,sum(uni),costo_u,sum(costo) from concentrado_2_3_4_total where afiliado='"+prog+"' and fecha between '"+ran1+"' and '"+ran2+"' and partida='1' group by clave order by (clave+0)");
		
       rset_002=stmt_002.executeQuery("select sum(uni),sum(costo),costo_u from concentrado_2_3_4_total where afiliado='"+prog+"' and fecha between '"+ran1+"' and '"+ran2+"' and id_med='MEDICAMENTO'  and  partida='1' group by clave");
	}
		  ...
		  while (rset_002.next()) 
                  {
			          ...
			          ixt_uni = ixt_uni+Integer.parseInt(ixt_cad);
			     	  ixt_cad3=rset_002.getString("sum(costo)");
					  ixt_costo2= ixt_costo2+Double.parseDouble(ixt_cad3);
					  unitario=rset_002.getString("costo_u");
					  unitario1= unitario1+Double.parseDouble(unitario);
...
// Si el parámetro es Todos los programas y el insumo es Material de Curación
	if(prog.equals("Todos")){	
 	rset_002=stmt_002.executeQuery("select sum(uni),sum(costo),sum(costo_u) from concentrado_2_3_4_total where fecha between '"+ran1+"' and '"+ran2+"' and id_med='MATERIAL DE CURACION'  and  partida='1' group by clave");
	}else{
	
	rset_002=stmt_002.executeQuery("select sum(uni),sum(costo),sum(costo_u) from concentrado_2_3_4_total where afiliado='"+prog+"' and fecha between '"+ran1+"' and '"+ran2+"' and id_med='MATERIAL DE CURACION'  and  partida='1' group by clave");
	}
		  // si rset_002 tiene datos hace los acumuladores
		  while (rset_002.next()) 
                  {
						ixt_cadmat=rset_002.getString("sum(uni)");
					  	ixt_unimat = ixt_unimat+Integer.parseInt(ixt_cadmat);
					  	ixt_cad3mat=rset_002.getString("sum(costo)");
						ixt_costo2mat= ixt_costo2mat+Double.parseDouble(ixt_cad3mat);
						unitariomat=rset_002.getString("sum(costo_u)");
				   		unitario1mat= unitario1mat+Double.parseDouble(unitariomat);
				  }	
...
<tr height="20">
		  ...
		  while (rset_001.next()) 
                  { 
					...
					ixt_costo= ixt_costo+Double.parseDouble(ixt_cad2);
					costo1=rset_001.getString("costo_u");
					costo2=costo2+Double.parseDouble(costo1);
				//out.print(rset_001.getString("clave") + " presentacion "+rset_001.getString("present")); 
...
<%
  ...
  // si se escogió Todos los programas, el valor lo tiene la variable prog
     if(prog.equals("Todos")){
             rset_001=stmt_001.executeQuery("select clave,descri,present,sum(uni),costo_u,sum(costo) from concentrado_2_3_4_total where fecha between '"+ran1+"' and '"+ran2+"' and partida='2' group by clave order by (clave+0)");
		
       rset_002=stmt_002.executeQuery("select sum(uni),sum(costo),costo_u from concentrado_2_3_4_total where fecha between '"+ran1+"' and '"+ran2+"' and id_med='MEDICAMENTO'  and  partida='2' group by clave");
	
}else{	
		rset_001=stmt_001.executeQuery("select clave,descri,present,sum(uni),costo_u,sum(costo) from concentrado_2_3_4_total where afiliado='"+prog+"' and fecha between '"+ran1+"' and '"+ran2+"' and partida='2' group by clave order by (clave+0)");
		
		rset_002=stmt_002.executeQuery("select sum(uni),sum(costo),costo_u from concentrado_2_3_4_total where afiliado='"+prog+"' and fecha between '"+ran1+"' and '"+ran2+"' and id_med='MEDICAMENTO'  and  partida='2' group by clave");
   ...
   //mientras haya valores que apunte rset_002
		  while (rset_002.next()) 
                  {
					ixt_cad=rset_002.getString("sum(uni)");
					ixt_uni3 = ixt_uni3+Integer.parseInt(ixt_cad);
					ixt_cad3=rset_002.getString("sum(costo)");
					ixt_costo2= ixt_costo2+Double.parseDouble(ixt_cad3);
					unitario=rset_002.getString("costo_u");
				   	unitario1= unitario1+Double.parseDouble(unitario);
	...
	if(prog.equals("Todos"))
   {//inicio del if	 
	rset_002=stmt_002.executeQuery("select sum(uni),sum(costo),costo_u from concentrado_2_3_4_total where fecha between '"+ran1+"' and '"+ran2+"' and id_med='MATERIAL DE CURACION'  and  partida='2' group by clave");
	
	}else{
	rset_002=stmt_002.executeQuery("select sum(uni),sum(costo),costo_u from concentrado_2_3_4_total where afiliado='"+prog+"' and fecha between '"+ran1+"' and '"+ran2+"' and id_med='MATERIAL DE CURACION'  and  partida='2' group by clave");
		  ...
		  while (rset_002.next()) 
                  {
					...
					ixt_cad3mat=rset_002.getString("sum(costo)");
					ixt_costo2mat= ixt_costo2mat+Double.parseDouble(ixt_cad3mat);
					unitariomat=rset_002.getString("costo_u");
				   	unitario1mat= unitario1mat+Double.parseDouble(unitariomat);
				  }	
				 
	if(prog.equals("Todos")){ 
	rset_002=stmt_002.executeQuery("select sum(uni),sum(costo),costo_u from concentrado_2_3_4_total where fecha between '"+ran1+"' and '"+ran2+"' and partida='2' group by clave");
	}else{	
	rset_002=stmt_002.executeQuery("select sum(uni),sum(costo),costo_u from concentrado_2_3_4_total where afiliado='"+prog+"' and fecha between '"+ran1+"' and '"+ran2+"' and partida='2' group by clave");
	}
		  while (rset_002.next()) 
                  {
					ixt_cadtotal=rset_002.getString("sum(uni)");
					ixt_unitotal = ixt_unitotal+Integer.parseInt(ixt_cadtotal);
					ixt_cad3total=rset_002.getString("sum(costo)");
					ixt_costo2total= ixt_costo2total+Double.parseDouble(ixt_cad3total);
					unitariototal=rset_002.getString("costo_u");
				   	unitario1total= unitario1total+Double.parseDouble(unitariototal);
				  }		  
...
<tr height="20">
          ...
          <tr height="20">
            ...
            <td height="26" align="right">&nbsp;<span class="style21"><%=formateador.format(unitario1total)%></span></td>
		  ...
		  <tr height="20">
            ...
            <td height="30" background="fndo3.jpg" ><div align="center" >TOTAL DE PIEZAS (SSN+SAVI) </div></td>
            <td height="30" background="fndo3.jpg" class="style22">&nbsp;</td>
...
FALTA CONTABILIZAR BASE DE DATOS DE HOSPITAL GENERAL GOMEZ PALACIO (ALMACEN) SEPTIEMBRE<br /><br />
<br />
  DERECHOS RESERVADOS GNKL/DESARROLLOS WEB©2013, PARA USO EXCLUSIVO DE SAVI/SSN<br />
  VERSION 1.2 </h5>
<p align="center"><br /></p>
<map name="Map2" id="Map2"><area shape="rect" coords="5,4,53,38" href="reporte2_conc_total.jsp?uni=<%=uni%>&rango1=<%=ran1%>&bd=<%=bd_jv%>&rango2=<%=ran2%>&programa=<%=prog%>&reporte=<%=rep%>&ban=<%=ban_jv%>&boton=EXECUTIVE" />
</map>
</body>
</html>
